...
class Store {
  ...
  action(name, ...args) {
    ...
    if (args.length === 2) {
      ...
      const cb = args[1];
      callback = (state, data) => {
        const recursivelyAssignObjects = (
          object,
          selectedObj,
          newValue,
        ) => {
          ...
          for (const key in object) {
            if (object.hasOwnProperty(key)) {
              ...
              if (
                ...
                const newObject = recursivelyAssignObjects(
                  object[key],
                  selectedObj,
                  newValue,
                );
        ...
        if (typeof selectedObj !== 'object') {
          console.warn(
            `Action selector must select an object! (${selectedObj}) selected`,
          );
        }

        const newState = recursivelyAssignObjects(
          state,
          selectedObj,
          cb(selectedObj, data),
        );
        if (!newState) {
          console.warn(
            'selector does not match any object',
            selector,
            state,
          );
        }
...
class StoreManager {
      ...
      if (this.stores[storeId]) {
        for (const id in this.stores[storeId].listeners) {
          if (this.stores[storeId].listeners[id]) {
            this.stores[storeId].listeners[id](
              this.stores[storeId].state,
            );
          }
        }
      }
  ...
  runMiddleware(action, data, store) {
    ...
    const middlewareAPI = {
      ...
      // TODO: Figure out why duplication here
      dispatch: (subAction, subData) => store.dispatch(subAction, subData), // allow for further dispatches in middleware //todo check this if we need to bind
    };
    ...
    function executeAction(name, subData) {
      if (!this.actions.hasOwnProperty(name)) {
        console.warn(`action not found: ${name}`);
      }
      return this.actions[name](this.state, subData);
    }
    ...
    const runMiddleware = [
      ...chain,
      executeAction.bind(store),
    ].reduceRight((composed, f) => f(composed));
