...
import storeManager from './robot/State';
import robotStore from './robot/Robot';
import { scene, camera, renderer } from './robot/THREEScene';
...
function TargetInit() {
   ...
   */
	...
	store.action('SET_EULER_RINGS_VISIBLE', (state, data) =>
		Object.assign({}, state, {
			eulerRingsVisible: data
		})
	);
	store.action('SET_CONTROL_VISIBLE', (state, data) =>
		Object.assign({}, state, {
			controlVisible: data
		})
	);

	const sphereGeo = new CylinderGeometry(1, 1, 1 * 2, 32);
	const target = new Group();

	const targetCylinder = new Mesh(
		sphereGeo,
	...
	function createRing(radius, color, axis) {
		...
		const circleMesh = new Mesh(
			new TorusGeometry(radius, 0.05, 6, 50),
			ringMaterial
		);

		const sphereMesh = new Mesh(
			new SphereGeometry(sphere_radius, 12, 10),
			ringMaterial
		);
		sphereMesh.position.x = radius;

		const composite = new Object3D();
	...
	function createAllRings(parentObject) {
		// debugger
		// create Rings
		ringx = createRing(2.0, 0xff0000, 'x');
		ringy = createRing(1.75, 0x00ff00, 'y');
		ringz = createRing(1.5, 0x0000ff, 'z');
	...
	store.listen(
		[() => store.getStore('Robot').getState().target, state => state],
		(targetT, state) => {
			...
			if (true) {
				// loop -  changing mode triggers change....
				disableUpdate = true;
	...
	control.addEventListener('change', () => {
		if (!disableUpdate) {
			// changing controlmode causes a loop
			targetChangedAction();
		}
	});
	...
	store.action(
		'CONTROL_SPACE_TOGGLE',
		state => state.controlSpace,
		controlSpace => (controlSpace === 'local' ? 'world' : 'local')
	);
	...
	store.action('CHANGE_CONTROL_MODE', (state, data) => ({
		...state,
		controlMode: data
	}));
	...
	store.action('TARGET_CHANGE_TARGET', (state, data) => {
		...
		if (state.followTarget) {
			store.getStore('Robot').dispatch('ROBOT_CHANGE_TARGET', {
				position: {
					...state.position, // allow for changing only one parameter (x,y,z)
					...data.position
				},
				rotation: {
					...state.rotation,
