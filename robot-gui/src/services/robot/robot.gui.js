...
for (const key in anglesDeg) {
  anglesGui
    ...
    .onChange(() => {
      const anglesRad = {};
      for (const subKey in anglesDeg) {
        if (anglesDeg.hasOwnProperty(subKey)) {
          anglesRad[subKey] = anglesDeg[subKey] * DEG_TO_RAD;
        }
      }
...
// TODO: Find out what is exported here
