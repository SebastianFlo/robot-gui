...
import THREERobot from '../THREERobot.service';

const THREESimulationRobot = new Group();
...
robotStore.listen(
  [state => state.geometry, state => state.jointLimits],
  (geometry, jointLimits) => {
...
robotStore.listen((state) => {
  ...
  for (let i = 0; i < 6; i++) {
    // do some caching
    if (!cacheState.jointOutOfBound[i] && state.jointOutOfBound[i]) {
      // highlight only on change
...
function buildRobot(state) {
  if (
    state.geometry.V3.y !== 0 ||
    state.geometry.V3.z !== 0 ||
    state.geometry.V4.y !== 0 ||
    state.geometry.V4.x !== 0
  ) {
    console.error('geometry where V3 y,z not 0 and V4 x,z not 0 are not supported, yet');
    [state.geometry.V3.y, state.geometry.V3.z, state.geometry.V4.y, state.geometry.V4.x] = [0, 0, 0, 0];
  }
