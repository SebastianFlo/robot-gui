...
import gui from './robot/UiDat';
import Kinematic from './robot/Kinematic';
import { scene } from './robot/THREEScene';
...
workingSpaceStore.action(
  'ADD_POSE_TO_EULER',
  state => state.directions,
  (directions, pose) => [...directions, pose],
);
...
workingSpaceStore.action('CHANGE_VISIBILITY', (state, data) =>
  Object.assign({}, state, {
    show: data,
  }),
);
...
robotStore.listen(
  [
    () => workingSpaceStore.getState(),
    state => state.jointLimits,
    state => state.geometry,
  ],
  (state, jointLimits, geometry) => {
    // gui
    ...
    if (state.show) {
      ...
      const IK = new Kinematic(geo, [
        // todo remove when using new ik
        [-360, 360],
        [-360, 360],
        [-360, 360],
        [-360, 360],
      ...
      const THREEgeo = new BoxBufferGeometry(0.1, 0.1, 0.1);

      for (let j = 0; j < euler.length; j++) {
        const { rx, ry, rz, color } = euler[j];
          ...
          for (let x = -i; x <= i; x += step) {
            for (let y = -i; y <= i; y += step) {
              for (let z = -i; z <= i; z += step) {
                ...
                const angles = [];
                IK.calculateAngles(x, y, z, rx, ry, rz, angles);

                let inReach = true;
                Object.keys(jointLimits).forEach((key, jointIndex) => {
                  const jl0 = jointLimits[key][0];
                  const jl1 = jointLimits[key][1];

                  if (
                    isNaN(angles[jointIndex]) ||
                    jointLimits[key][0] > angles[jointIndex] ||
                    jointLimits[key][1] < angles[jointIndex]
                  ) {
                    inReach = false;
                  }
                });
